        ...
        with closing(self.conn.cursor()) as cr:
            try:
                cr.execute("SELECT 1 FROM ir_module_module "
                           ...
                           ('odoo_magento2_ept', 'installed'))
            except psycopg2.ProgrammingError as err:
                if str(err).startswith('relation "ir_module_module" '
                                           'does not exist'):
                    return False
                else:
                    raise
    ...
    def initialize_databases(self):
        for db_name in self.get_db_names():
            ...
            if not db.has_connector:
                ...
                for job_data in  db.select_jobs('state in %s', (('started','enqueued'),)) :
                    job_uuid = job_data[1]
                    job = self.channel_manager._jobs_by_uuid.get(job_uuid)
                    db.set_job_pending(job_uuid)
