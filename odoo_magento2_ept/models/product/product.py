...
from odoo.addons.odoo_magento2_ept.models.unit.mapper import (mapping,only_create,ImportMapper,ExportMapper,changed_by)
...
class MagentoProductProduct(models.Model):
    ...
    product_url_ids = fields.One2many(
        'magento.product.url','magento_product_id', string='Product Url')
    ...
    def run(self, filters=None):
        ...
        for record_id in product_record_ids : 
            if product_record_ids.get(record_id) == 'configurable':
                import_product_record.delay(self.session,'magento.product.template',self.backend_record.id,record_id,is_configurable=True,priority=1)
            else : 
...
class ProductImporter(MagentoImporter):
    ...
    def _get_product_vals(self,data):
        ...
        vals = {}
        for product in products :
            vals.update({'erp_id':product.id,'name' : product.name})
    ...
    def create_product_url(self,storeview,binding,url):
        product_url_obj = self.env['magento.product.url']
        magento_product_url=product_url_obj.search([('storeview_id','=',storeview.id),('product_url','=',url)])
        if not magento_product_url:
            product_url_obj.create({
                                        'magento_product_id' : binding.id,
                                        'website_id' : storeview.website_id.id,
    ...
    def _after_import(self, binding):
        ...
        magento_website_ids = record['extension_attributes'].get('website_ids',[])
        website_ids = self.env['magento.website'].search([('magento_id','in',magento_website_ids)])
        traslated_record = price_importer.run(self.magento_id, binding.id,website_ids,present_record=traslated_record)
        #products_url = record['custom_attributes'].get('attribute_code',[])
        ...
        for storeview,lang_record in traslated_record.items():
            product_url = lang_record.get('custom_attributes').get('products_url')
            if product_url :
                self.create_product_url(storeview,binding,product_url)
...
class ProductExporter(MagentoExporter):
    ...
    def export_translation_to_magento(self,website_ids,binding_record):
        ...
        for storeview in storeviews:
            ...
            translated_value.update({'name' : binding_record.with_context(lang=lang).magento_product_name})
            export_data = {
