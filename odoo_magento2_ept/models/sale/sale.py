...
class SaleOrder(models.Model):
    ...
    #Below Field changes by Harsh Parekh
...
#             magento_payment_method = self.env['magento.payment.method'].search([('payment_method_code','=',payment_method),('backend_id','=',self.backend_record.id)],limit=1)
    ...
    def website(self,record):
        ...
        website_binder = self.binder_for('magento.website')
        oe_website_id = website_binder.to_openerp(website_id)
        global_channel_id = self.env['magento.website'].browse(oe_website_id).global_channel_id
        assert oe_website_id, ("Website is missing!")
        return {'website_id':oe_website_id,'global_channel_id':global_channel_id.id}
    ...
    def _import_dependencies(self):
        ...
        for line in items:
            ...
            if 'product_id' in line:
                magento_product = self.env['magento.product.product'].get_magento_product(line.get('sku'),self.backend_record.id)
                if not magento_product:
                    try :
                        self._import_dependency(line['product_id'],'magento.product.product')
...
class SaleOrderLineImportMapper(ImportMapper):
    ...
    @mapping
    def product_url(self,record):
        store_view = self.options.get('storeview')
        sku = record.get('sku')
        product_id = self.env['magento.product.product'].get_magento_product(sku,self.backend_record.id)
        url_obj = self.env['magento.product.url'].search([('magento_product_id','=',product_id.id),('storeview_id','=',store_view.id)],limit=1)
        return {'producturl' : url_obj and url_obj.product_url}
