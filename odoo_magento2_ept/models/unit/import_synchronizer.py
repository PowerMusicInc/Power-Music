...
from odoo.addons.odoo_magento2_ept.models.unit.synchronizer import Importer
...
#from odoo.addons.odoo_magento2_ept.models.backend.related_action import link
...
class MagentoImporter(Importer):
    ...
    def run(self, magento_id, force=False):
        ...
        return _("Record is imported with Id %s"%(binding))
...
class ProductPriceImporter(Importer):
    ...
    def run(self, magento_id, binding_id, website_ids ,mapper_class=None, present_record=False):
        self.magento_id = magento_id
        storeviews = self.env['magento.storeview'].search(
            [('backend_id', '=', self.backend_record.id),('website_id','in',website_ids.ids)]
        )
        default_lang = self.backend_record.default_lang_id
        # Remove it Because Product Price we need to set for every Product, It is not based on language.
...
#             return
        ...
        for storeview in storeviews:
            lang_record = False
            if present_record:
                lang_record = present_record.get(storeview,False)
            if not lang_record:
                lang_record = self._get_magento_data(storeview.magento_id)
           
            lang_record_dict.update({storeview:lang_record})
          
            
        return lang_record_dict
...
class TranslationImporter(Importer):
    ...
    def run(self, magento_id, binding_id, mapper_class=None):
        ...
        for storeview in lang_storeviews:
            ...
            binding.with_context(lang=storeview.lang_id.code).write(data)
